...
use ggez::graphics::{DrawMode, Image, Point2};
use ggez::*;
...
impl Bitmap {
    fn new(width: u32, height: u32) -> Bitmap {
        Bitmap {
            raw: vec![0xff; (width * height * 4) as usize],
            width: width,
            height: height,
            dirty: true,
        }
    ...
    fn from_image(context: &mut Context, image: Image) -> Bitmap {
        Bitmap {
            raw: image.to_rgba8(context).unwrap(),
            width: image.width(),
            height: image.height(),
            dirty: true,
        }
...
mrusty_class!(Pixel, "Color", {
    ...
    def!("r", |mruby, slf: (&Pixel)| mruby.fixnum(slf.r as i32));

    def!("g", |mruby, slf: (&Pixel)| mruby.fixnum(slf.g as i32));

    def!("b", |mruby, slf: (&Pixel)| mruby.fixnum(slf.b as i32));

    def!("a", |mruby, slf: (&Pixel)| mruby.fixnum(slf.a as i32));
});
...
mrusty_class!(Input, "InputType", {
    def!("trigger?",
         |mruby, slf: (&Input), k: Value| if let Some(kc) = symbol2keycode(k.to_str().unwrap()) {
...
mrusty_class!(SpriteLike, "Sprite", {
    def!("initialize", |mruby, pth: Value| {
        let s = Rc::new(RefCell::new(Sprite::from_bitmap(Bitmap::from_path(pth.to_str()
            .unwrap()))));
        let graphics = mruby.run("RGSS::Graphics")
            .unwrap()
            .to_obj::<Graphics>()
            .unwrap();
    ...
    def!("bitmap",
         |mruby, slf: (&mut SpriteLike)| mruby.option(slf.bitmaplike()));

    def!("bitmap=",
         |mruby, slf: (&mut SpriteLike), v: (&BitmapLike)| {
...
mrusty_class!(BitmapLike, "Bitmap", {
    ...
    def!("get_pixel",
         |mruby, slf: (&mut BitmapLike), x: i32, y: i32| mruby.obj(slf.get_pixel(x, y)));

    def!("set_pixel",
         |mruby, slf: (&mut BitmapLike), x: i32, y: i32, p: Value| {
    ...
    def!("fill_rect",
         |mruby, slf: (&mut BitmapLike), x: i32, y: i32, width: i32, height: i32, p: Value| {
             let p_ = p.to_obj::<Pixel>().unwrap();
...
impl event::EventHandler for MainState {
    fn update(&mut self, _ctx: &mut Context) -> GameResult<()> {
        ...
        let graphics = self.mruby
            .run("RGSS::Graphics")
            .unwrap()
            .to_obj::<Graphics>()
            .unwrap();
        graphics.borrow_mut().update(_ctx);
        let input = self.mruby
            .run("RGSS::Input")
            .unwrap()
            .to_obj::<Input>()
            .unwrap();
